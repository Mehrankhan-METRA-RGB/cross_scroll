...
/// See also:
///
///   * [DiagnalScrollTest.builder], whose builder is of this type.
...
/// {@end-tool}
@immutable
class DiagnalScrollTest extends StatefulWidget {
  ...
  ///
  /// Passed with the [DiagnalScrollTest.builder] constructor.
  ///
  /// {@tool dartpad}
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      DiagnalScrollTest.getNearestPointOnLine(point, quad.point0, quad.point1),
      DiagnalScrollTest.getNearestPointOnLine(point, quad.point1, quad.point2),
      DiagnalScrollTest.getNearestPointOnLine(point, quad.point2, quad.point3),
      DiagnalScrollTest.getNearestPointOnLine(point, quad.point3, quad.point0),
    ];
  ...
  @override
  State<DiagnalScrollTest> createState() => _DiagnalScrollTestState();
}

class _DiagnalScrollTestState extends State<DiagnalScrollTest>
  ...
  @override
  void didUpdateWidget(DiagnalScrollTest oldWidget) {
...
Quad _getAxisAlignedBoundingBoxWithRotationCross(Rect rect, double rotation) {
  ...
  return DiagnalScrollTest.getAxisAlignedBoundingBox(boundariesRotated);
}
...
Offset _exceedsByCross(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside =
        DiagnalScrollTest.getNearestPointInside(point, boundary);
...
/// This enum is used to specify the behavior of the [DiagnalScrollTest] when
